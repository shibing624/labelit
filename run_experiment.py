...
See scorer/create_data.py for all available datasets.
...
from sampler.constants import get_AL_sampler
from sampler.constants import get_wrapper_AL_mapping
...
flags.DEFINE_string("save_dir", "./output/toy_experiments",
...
def generate_one_curve(X,
    ...
    Args:
      X: training data
      y: training labels
      sampler: sampling class from sampler, assumes reference
    ...
    # TODO: add option to find best hyperparameter setting first on
    ...
    # TODO: remove mixture parameter and have the mixture be specified as
    ...
    if warmstart_size < 1:
        ...
        # TODO: switch to sklearn stratified sampler
    ...
    indices, X_train, y_train, X_val, y_val, X_test, y_test, y_noise = (
        score_methods.get_train_val_test_splits(X, y, max_points, seed, confusion,
    ...
    for b in range(n_batches):
        ...
        # print('select_batch_inputs:', select_batch_inputs)
...
def main():
    ...
    if do_save:
        ...
        sys.stdout = score_methods.Logger(filename)
    ...
    X, y = score_methods.get_mldata(FLAGS.data_dir, FLAGS.dataset)
    ...
    for c in confusions:
        for m in mixtures:
            for seed in range(starting_seed, starting_seed + FLAGS.trials):
                ...
                score_model = score_methods.get_model(FLAGS.score_method, seed)
                if (FLAGS.select_method == "None" or
                            FLAGS.select_method == FLAGS.score_method):
                    select_model = None
                else:
                    select_model = score_methods.get_model(FLAGS.select_method, seed)
